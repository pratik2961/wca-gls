...
if uploaded_file is not None:
    if ".txt" in uploaded_file.name:
        ...
        # st.text(data)
        df,format_check = preprocessor.preprocess(data)
        if format_check == False:
            st.error(" -> You have uploaded file with wrong Time format.")
            st.error(" -> WCA require only 24 Hour formated text file")
            st.error(" -> Note 'AM/PM format not working here.....'")
            st.error(" -> 😥 ")
        ...
        st.error(" -> You have uploaded file with wrong extension.")
        st.error(" -> WCA require only text file created by whatsapp for Analysis")
        st.error(" -> 😁 ")